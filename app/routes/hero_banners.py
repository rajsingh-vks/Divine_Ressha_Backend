...
from app.schemas.hero_banners import HeroBannerOut
...
def _serialize_banner(document: dict, request: Request) -> HeroBannerOut:
...
async def _save_uploaded_image(banner_id: ObjectId, image: UploadFile) -> str:
    ...
    return storage.save_banner_image(
...
async def create_hero_banner(
    ...
    banner_id = ObjectId()
    image_url = await _save_uploaded_image(banner_id, image)
    ...
    await request.app.state.mongo_db.hero_banners.insert_one(document)
    return _serialize_banner(document, request)
...
async def list_hero_banners(
    ...
    cursor = (
        request.app.state.mongo_db.hero_banners.find(query)
        .sort([("display_order", 1), ("created_at", -1)])
        .limit(limit)
    )
    items = await cursor.to_list(length=limit)
    return [_serialize_banner(item, request) for item in items]
...
async def update_hero_banner(
    ...
    banner_obj_id = _to_object_id(banner_id)
    existing = await db.hero_banners.find_one({"_id": banner_obj_id})
    if not existing:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Hero banner not found.")
    ...
    if image and image.filename:
        storage.delete_banner_media(str(banner_obj_id))
        updates["image_url"] = await _save_uploaded_image(banner_obj_id, image)

    if not updates:
        return _serialize_banner(existing, request)

    updates["updated_at"] = datetime.now(UTC)
    await db.hero_banners.update_one({"_id": banner_obj_id}, {"$set": updates})
    updated = await db.hero_banners.find_one({"_id": banner_obj_id})
    return _serialize_banner(updated, request)
...
async def delete_hero_banner(
    ...
    banner_obj_id = _to_object_id(banner_id)
    result = await request.app.state.mongo_db.hero_banners.delete_one({"_id": banner_obj_id})
